...
class TowerProtocol:
    def __init__(self, host: str = "0.0.0.0", port: int = 9876):
        max_workers = os.cpu_count() or 8 # Fallback to 4 if core count can't be determined
        self.host = host
        self.port = port
    ...
    def _process_incoming_packet(self, data: bytes, addr: tuple):
        try:
            ...
            if 'TotalNumberOfFragments' in message_dict and 'UniqueSequencedID' in message_dict:
                # Submit the request handling to a worker thread
                self.executor.submit(self._handle_sign_detection_request, message_dict, addr)
            elif 'fragmentSequenceID' in message_dict and 'fragment' in message_dict and 'UniqueSequencedID' in message_dict:
                # Submit the fragment handling to a worker thread
                self.executor.submit(self._handle_image_fragment, message_dict, addr)
            else:
